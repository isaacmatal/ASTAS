...
Public Class Cooperativa_Estado_Cuenta_ATAF
    ...
    Dim nombre_asociacion_ As String
...
#Region "Eventos"
    Private Sub Cooperativa_Estado_Cuenta_Load(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles MyBase.Load
        ...
        Dim TDatosGral As New DataTable("DatosGenerales")
        sqlCmd.CommandText = "SELECT NOMBRE_COMPAÑIA FROM dbo.CATALOGO_COMPAÑIAS WHERE COMPAÑIA = 2"
        TDatosGral = jClass.obtenerDatos(sqlCmd)
        nombre_asociacion_ = TDatosGral.Rows(0).Item("NOMBRE_COMPAÑIA").ToString()
        Iniciando = False
    End Sub

    Private Sub cmbGerencias_SelectedIndexChanged_1(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles cmbGerencias.SelectedIndexChanged
    ...
    Private Sub cmbDivisiones_SelectedIndexChanged_1(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles cmbDivisiones.SelectedIndexChanged
        If Iniciando = False Then
            jClass.CargarCombo(Me.cmbDepartamentos, "SELECT DEPARTAMENTO,DESCRIPCION_DEPARTAMENTO FROM COOPERATIVA_CATALOGO_DEPARTAMENTOS where division=" & Me.cmbDivisiones.SelectedValue.ToString() & " ORDER BY DESCRIPCION_DEPARTAMENTO ASC", "DEPARTAMENTO", "DESCRIPCION_DEPARTAMENTO")
    ...
    Private Sub cmbDepartamentos_SelectedIndexChanged_1(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles cmbDepartamentos.SelectedIndexChanged
        If Iniciando = False Then
            jClass.CargarCombo(Me.cmbSecciones, "SELECT SECCION,DESCRIPCION_SECCION FROM COOPERATIVA_CATALOGO_SECCIONES where division=" & Me.cmbDivisiones.SelectedValue.ToString() & " and departamento=" & cmbDepartamentos.SelectedValue.ToString() & " ORDER BY DESCRIPCION_SECCION ASC", "SECCION", "DESCRIPCION_SECCION")
            If GroupBox7.Enabled = True Then
    ...
    Private Sub cmbSecciones_SelectedIndexChanged_1(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles cmbSecciones.SelectedIndexChanged
        If Iniciando = False Then
    ...
    Private Sub cmbCargos_SelectedIndexChanged_1(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles cmbCargos.SelectedIndexChanged
        If Iniciando = False Then
            If GroupBox7.Enabled = True Then
    ...
    Private Sub Button1_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles Button1.Click
        ...
        txtObj = Rpt.Section2.ReportObjects.Item("txtAsoc")
        txtObj.Text = "A.T.A.F."
        txtObj = Rpt.Section2.ReportObjects.Item("txtPie")
        txtObj.Text = "Cualquier duda en su estado consultar en oficinas de ATAF al Tel.2243-0333 ext. 1555 o al Tel.2202-1000 ext. 3504"
        SetTitleReport(Rpt)
        If TextBox1.Text <> "" Then
            Try
                sql = "EXEC sp_COOPERATIVA_ESTADO_CUENTA "
                sql &= "@CODIGO_EMPLEADO = " & TextBox1.Text
                ...
                If cmbGeneral.Checked = True Then
                    sql = "EXECUTE sp_COOPERATIVA_REPORTE_CONSTANCIA_DEUDAS  @COMPAÑIA = 1,@CodigoBuxis=" & TextBox1.Text & ",@FECHA_HASTA ='" & Format(DateTimePicker3.Value, "dd-MM-yyyy hh:mm:ss") & "' "
                Else
                    sql = "EXECUTE [dbo].[COOPERATIVA_ESTADO_CUENTA_DEUDA_SOCIOS] @COMPAÑIA = " & Compañia & ", @DEDUCCION = 0, @CodigoBuxis =" & TextBox1.Text & ", @FECHA_DESDE = '" & Format(DateTimePicker1.Value, "dd-MM-yyyy hh:mm:ss") & "', @FECHA_HASTA = '" & Format(DateTimePicker2.Value, "dd-MM-yyyy hh:mm:ss") & "'"
                End If
    ...
    Private Sub Cooperativa_Estado_Cuenta_Paint(ByVal sender As System.Object, ByVal e As System.Windows.Forms.PaintEventArgs) Handles MyBase.Paint
    ...
    Private Sub TextBox1_KeyPress(ByVal sender As System.Object, ByVal e As System.Windows.Forms.KeyPressEventArgs) Handles TextBox1.KeyPress
        If e.KeyChar = Microsoft.VisualBasic.ChrW(13) And Me.TextBox1.Text.Length > 0 Then
            Me.TextBox2.Text = jClass.obtenerEscalar("SELECT NOMBRE_COMPLETO FROM COOPERATIVA_CATALOGO_SOCIOS WHERE COMPAÑIA = " & Compañia & " AND CODIGO_EMPLEADO = " & Me.TextBox1.Text)
    ...
    Private Sub Button4_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles Button4.Click
        ...
        Rpt.Section3.ReportObjects.Item("Picture2").ObjectFormat.EnableSuppress = True
        Rpt.Section3.ReportObjects.Item("txtFirma").ObjectFormat.EnableSuppress = False
        generaQuery()
        sqlCmd.CommandText = sql
        Table = jClass.obtenerDatos(sqlCmd)
        Rpt.SetDataSource(Table)
    ...
    Private Sub Button5_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles Button5.Click
        ...
        sql &= "   AND s1.ORIGEN <> 18 AND SA.ASOCIACION = 2 AND SA.NUMERO_REPARTO = " & Me.NumericUpDown11.Value & vbCrLf
        ...
        If Table1.Rows.Count > 0 Then
            For i As Integer = 0 To Table1.Rows.Count - 1
                ...
                Try
                    Rpt.Subreports.Item(0).SetDataSource(Table)
                Catch EX As Exception
                    MsgBox(EX.Message, MsgBoxStyle.Information, "Estado Cuenta")
                    'MsgBox(EX.Message & vbCrLf & EX.InnerException.Message)
                End Try
                If CheckBox1.Checked = True Then
                ...
                Else
                    ...
                    Catch EX As Exception
                        MsgBox(EX.Message, MsgBoxStyle.Information, "Separador")
                    End Try
        ...
        If Table1.Rows.Count > 0 Then
            For i As Integer = 0 To Table1.Rows.Count - 1
                ...
                If CheckBox1.Checked = True Then
                    ...
                    Lista.Add(My.Computer.FileSystem.SpecialDirectories.MyDocuments & "\ESTADOS_DE_CUENTA\" & cmbDivisiones.Text & "\" & Table1.Rows(i).Item(0).ToString() & ".pdf")
                Else
                    If System.IO.Directory.Exists(My.Computer.FileSystem.SpecialDirectories.MyDocuments & "\ESTADOS_DE_CUENTA") Then
                    Else
                        System.IO.Directory.CreateDirectory(My.Computer.FileSystem.SpecialDirectories.MyDocuments & "\ESTADOS_DE_CUENTA")
                    End If
                    Rpt.ExportToDisk(CrystalDecisions.Shared.ExportFormatType.PortableDocFormat, My.Computer.FileSystem.SpecialDirectories.MyDocuments & "\ESTADOS_DE_CUENTA\" & Table1.Rows(i).Item(0).ToString() & ".pdf")
            ...
            Dim sFileJoin As String = My.Computer.FileSystem.SpecialDirectories.MyDocuments & "\ESTADOS_DE_CUENTA\EdoCtaDividendosATAF.pdf"
            ...
            Try
                ...
                Shell("cmd /C """ & My.Computer.FileSystem.SpecialDirectories.MyDocuments & "\ESTADOS_DE_CUENTA\EdoCtaDividendosATAF.pdf""")
    ...
    Private Sub SetTitleReport(ByRef reporte_ As Object)
        Dim txtObj As CrystalDecisions.CrystalReports.Engine.TextObject
        'Dim imgObj As CrystalDecisions.CrystalReports.Engine.PictureObject
        txtObj = reporte_.Section2.ReportObjects.Item("Text12")
        reporte_.Section2.ReportObjects.Item("Picture1").ObjectFormat.EnableSuppress = True
        'imgObj = reporte_.Section2.ReportObjects.Item("Picture1")
        txtObj.Text = nombre_asociacion_
        'imgObj.Height = 0
    End Sub
    ...
    Private Sub btnDetGral_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles btnDetGral.Click
        Dim respuesta As MsgBoxResult
...
#Region "Funciones"
        ...
        If numSocio <> "" Then
            Try
                ...
                If Table.Rows.Count = 1 Then
                    ...
                    Me.TextBox2.Text = Table.Rows(0).Item("NOMBRE_COMPLETO")
                    TextBox1.Focus()
                Else
                    MsgBox("No se encontraron datos para el socio: " + Me.TextBox1.Text, MsgBoxStyle.Critical, "No hay Datos")
                End If
            Catch ex As Exception
        ...
        If Iniciando = False Then
            ...
            Try
                If CheckBox1.Checked = True Then
                    ...
                    If Me.RadioButton1.Checked = True Then
                        Table1 = jClass.ejecutaSQLl_llenaDTABLE("select distinct s.CODIGO_EMPLEADO, s1.DEPARTAMENTO from COOPERATIVA_SOCIO_AHORROS s INNER JOIN COOPERATIVA_DETALLE_DIVIDENDOS_POR_SOCIO SA ON SA.COMPAÑIA=SA.COMPAÑIA AND SA.CODIGO_EMPLEADO=s.CODIGO_EMPLEADO inner join COOPERATIVA_CATALOGO_SOCIOS s1 on s.CODIGO_EMPLEADO=s1.CODIGO_EMPLEADO where s1.departamento=" & cmbDepartamentos.SelectedValue.ToString() & " and s1.division=" & Me.cmbDivisiones.SelectedValue.ToString() & " order by CODIGO_EMPLEADO asc")
                        cantidadEmpleados.Text = jClass.leerDataeader("SELECT COUNT(DISTINCT S.CODIGO_EMPLEADO) FROM COOPERATIVA_CATALOGO_SOCIOS S INNER JOIN COOPERATIVA_DETALLE_DIVIDENDOS_POR_SOCIO C ON C.COMPAÑIA=S.COMPAÑIA AND C.CODIGO_EMPLEADO=S.CODIGO_EMPLEADO WHERE S.DIVISION=" & Me.cmbDivisiones.SelectedValue.ToString(), 0)
                        'SEGUN COMBO DE DEPARTAMENTOS
                    ElseIf Me.RadioButton2.Checked = True Then
                        If TextBox3.Text = "" Then
                            Table1 = jClass.ejecutaSQLl_llenaDTABLE("select distinct s.CODIGO_EMPLEADO, s1.DEPARTAMENTO from COOPERATIVA_SOCIO_AHORROS s INNER JOIN COOPERATIVA_DETALLE_DIVIDENDOS_POR_SOCIO SA ON SA.COMPAÑIA=SA.COMPAÑIA AND SA.CODIGO_EMPLEADO=s.CODIGO_EMPLEADO inner join COOPERATIVA_CATALOGO_SOCIOS s1 on s.CODIGO_EMPLEADO=s1.CODIGO_EMPLEADO where s1.departamento=" & cmbDepartamentos.SelectedValue.ToString() & " and s1.division=" & Me.cmbDivisiones.SelectedValue.ToString() & " and s1.departamento=" & cmbDepartamentos.SelectedValue.ToString() & " order by CODIGO_EMPLEADO asc")
                        Else
                            Table1 = jClass.ejecutaSQLl_llenaDTABLE("select distinct s.CODIGO_EMPLEADO, s1.DEPARTAMENTO from COOPERATIVA_SOCIO_AHORROS s INNER JOIN COOPERATIVA_DETALLE_DIVIDENDOS_POR_SOCIO SA ON SA.COMPAÑIA=SA.COMPAÑIA AND SA.CODIGO_EMPLEADO=s.CODIGO_EMPLEADO inner join COOPERATIVA_CATALOGO_SOCIOS s1 on s.CODIGO_EMPLEADO=s1.CODIGO_EMPLEADO where s1.departamento=" & cmbDepartamentos.SelectedValue.ToString() & " and s1.division=" & Me.cmbDivisiones.SelectedValue.ToString() & " and s1.departamento=" & cmbDepartamentos.SelectedValue.ToString() & " AND SA.CODIGO_EMPLEADO>" & Val(TextBox3.Text) & " order by CODIGO_EMPLEADO asc")
                        End If
                        cantidadEmpleados.Text = jClass.leerDataeader("SELECT COUNT(DISTINCT S.CODIGO_EMPLEADO) FROM COOPERATIVA_CATALOGO_SOCIOS S INNER JOIN COOPERATIVA_DETALLE_DIVIDENDOS_POR_SOCIO C ON C.COMPAÑIA=S.COMPAÑIA AND C.CODIGO_EMPLEADO=S.CODIGO_EMPLEADO WHERE S.DIVISION=" & Me.cmbDivisiones.SelectedValue.ToString() & " AND S.DEPARTAMENTO=" & Me.cmbDepartamentos.SelectedValue.ToString(), 0)
                        'SEGUN COMBO DE SECCIONES
                    ElseIf Me.RadioButton3.Checked = True Then
                        If TextBox3.Text = "" Then
                            Table1 = jClass.ejecutaSQLl_llenaDTABLE("select distinct s.CODIGO_EMPLEADO, s1.DEPARTAMENTO from COOPERATIVA_SOCIO_AHORROS s INNER JOIN COOPERATIVA_DETALLE_DIVIDENDOS_POR_SOCIO SA ON SA.COMPAÑIA=SA.COMPAÑIA AND SA.CODIGO_EMPLEADO=s.CODIGO_EMPLEADO inner join COOPERATIVA_CATALOGO_SOCIOS s1 on s.CODIGO_EMPLEADO=s1.CODIGO_EMPLEADO where s1.departamento=" & cmbDepartamentos.SelectedValue.ToString() & " and s1.division=" & Me.cmbDivisiones.SelectedValue.ToString() & " and s1.departamento=" & cmbDepartamentos.SelectedValue.ToString() & " AND s1.SECCION=" & cmbSecciones.SelectedValue.ToString() & " order by CODIGO_EMPLEADO asc")
                        Else
                            Table1 = jClass.ejecutaSQLl_llenaDTABLE("select distinct s.CODIGO_EMPLEADO, s1.DEPARTAMENTO from COOPERATIVA_SOCIO_AHORROS s INNER JOIN COOPERATIVA_DETALLE_DIVIDENDOS_POR_SOCIO SA ON SA.COMPAÑIA=SA.COMPAÑIA AND SA.CODIGO_EMPLEADO=s.CODIGO_EMPLEADO inner join COOPERATIVA_CATALOGO_SOCIOS s1 on s.CODIGO_EMPLEADO=s1.CODIGO_EMPLEADO where s1.departamento=" & cmbDepartamentos.SelectedValue.ToString() & " and s1.division=" & Me.cmbDivisiones.SelectedValue.ToString() & " and s1.departamento=" & cmbDepartamentos.SelectedValue.ToString() & " AND s1.SECCION=" & cmbSecciones.SelectedValue.ToString() & " AND SA.CODIGO_EMPLEADO>" & Val(TextBox3.Text) & " order by CODIGO_EMPLEADO asc")
                        End If
                        cantidadEmpleados.Text = jClass.leerDataeader("SELECT COUNT(DISTINCT S.CODIGO_EMPLEADO) FROM COOPERATIVA_CATALOGO_SOCIOS S INNER JOIN COOPERATIVA_DETALLE_DIVIDENDOS_POR_SOCIO C ON C.COMPAÑIA=S.COMPAÑIA AND C.CODIGO_EMPLEADO=S.CODIGO_EMPLEADO WHERE S.DIVISION=" & Me.cmbDivisiones.SelectedValue.ToString() & " AND S.DEPARTAMENTO=" & Me.cmbDepartamentos.SelectedValue.ToString() & " AND S.SECCION=" & Me.cmbSecciones.SelectedValue.ToString(), 0)
                        'SEGUN COMBO DE CARGOS
                    ElseIf Me.RadioButton4.Checked = True Then
                        If Me.cmbCargos.SelectedValue <> Nothing Then
                            If TextBox3.Text = "" Then
                                Table1 = jClass.ejecutaSQLl_llenaDTABLE("select distinct s.CODIGO_EMPLEADO, s1.DEPARTAMENTO from COOPERATIVA_SOCIO_AHORROS s INNER JOIN COOPERATIVA_DETALLE_DIVIDENDOS_POR_SOCIO SA ON SA.COMPAÑIA=SA.COMPAÑIA AND SA.CODIGO_EMPLEADO=s.CODIGO_EMPLEADO inner join COOPERATIVA_CATALOGO_SOCIOS s1 on s.CODIGO_EMPLEADO=s1.CODIGO_EMPLEADO where s1.departamento=" & cmbDepartamentos.SelectedValue.ToString() & " and s1.division=" & Me.cmbDivisiones.SelectedValue.ToString() & " and s1.departamento=" & cmbDepartamentos.SelectedValue.ToString() & " AND s1.SECCION=" & cmbSecciones.SelectedValue.ToString() & " and s1.CARGO=" & cmbCargos.SelectedValue.ToString() & " order by CODIGO_EMPLEADO asc")
                            Else
                                Table1 = jClass.ejecutaSQLl_llenaDTABLE("select distinct s.CODIGO_EMPLEADO, s1.DEPARTAMENTO from COOPERATIVA_SOCIO_AHORROS s INNER JOIN COOPERATIVA_DETALLE_DIVIDENDOS_POR_SOCIO SA ON SA.COMPAÑIA=SA.COMPAÑIA AND SA.CODIGO_EMPLEADO=s.CODIGO_EMPLEADO inner join COOPERATIVA_CATALOGO_SOCIOS s1 on s.CODIGO_EMPLEADO=s1.CODIGO_EMPLEADO where s1.departamento=" & cmbDepartamentos.SelectedValue.ToString() & " and s1.division=" & Me.cmbDivisiones.SelectedValue.ToString() & " and s1.departamento=" & cmbDepartamentos.SelectedValue.ToString() & " AND s1.SECCION=" & cmbSecciones.SelectedValue.ToString() & " and s1.CARGO=" & cmbCargos.SelectedValue.ToString() & " AND SA.CODIGO_EMPLEADO>" & Val(TextBox3.Text) & " order by CODIGO_EMPLEADO asc")
                            End If
                            cantidadEmpleados.Text = jClass.leerDataeader("SELECT COUNT(DISTINCT S.CODIGO_EMPLEADO) FROM COOPERATIVA_CATALOGO_SOCIOS S INNER JOIN COOPERATIVA_DETALLE_DIVIDENDOS_POR_SOCIO C ON C.COMPAÑIA=S.COMPAÑIA AND C.CODIGO_EMPLEADO=S.CODIGO_EMPLEADO WHERE S.DIVISION=" & Me.cmbDivisiones.SelectedValue.ToString() & " AND S.DEPARTAMENTO=" & Me.cmbDepartamentos.SelectedValue.ToString() & " AND S.SECCION=" & Me.cmbSecciones.SelectedValue.ToString() & " AND S.CARGO=" & Me.cmbCargos.SelectedValue.ToString(), 0)
                        Else
                            cantidadEmpleados.Text = 0
                            Table1 = Nothing
                        End If
                    ...
                    ElseIf RadioButton5.Checked = True Then
                        Table1 = jClass.ejecutaSQLl_llenaDTABLE("SELECT DISTINCT CODIGO_EMPLEADO, P.Departamento FROM COOPERATIVA_DETALLE_DIVIDENDOS_POR_SOCIO S INNER JOIN PLANILLA_CATALOGO_EMPLEADOS P ON P.CodigoEmpleado=S.CODIGO_EMPLEADO WHERE P.Gerencia='" & cmbGerencias.SelectedValue.ToString() & "'")
                        cantidadEmpleados.Text = jClass.leerDataeader("SELECT COUNT(DISTINCT CODIGO_EMPLEADO) FROM COOPERATIVA_DETALLE_DIVIDENDOS_POR_SOCIO S INNER JOIN PLANILLA_CATALOGO_EMPLEADOS P ON P.CodigoEmpleado=S.CODIGO_EMPLEADO WHERE P.Gerencia='" & cmbGerencias.SelectedValue.ToString() & "'", 0)
                    End If
                Else
                    Table1 = jClass.ejecutaSQLl_llenaDTABLE("SELECT DISTINCT SA.CODIGO_EMPLEADO,s1.NOMBRE_COMPLETO,s1.DIVISION,s1.DEPARTAMENTO,s1.SECCION FROM COOPERATIVA_DETALLE_DIVIDENDOS_POR_SOCIO SA INNER JOIN COOPERATIVA_CATALOGO_SOCIOS s1 ON SA.CODIGO_EMPLEADO=s1.CODIGO_EMPLEADO WHERE s1.ORIGEN <> 18 AND SA.ASOCIACION = 2 AND SA.NUMERO_REPARTO = " & Me.NumericUpDown11.Value & " ORDER BY s1.DIVISION,s1.DEPARTAMENTO,s1.SECCION,s1.NOMBRE_COMPLETO asc")
    ...
    Private Sub CrearPDF(ByVal lista_ As System.Collections.Generic.List(Of String))
        ...
        Dim sFileJoin As String = My.Computer.FileSystem.SpecialDirectories.Desktop & "\EdoCtaDividendosATAF.pdf"
        Dim Doc As New Document()
        Try
            Dim fs As New FileStream(sFileJoin, FileMode.Create, FileAccess.Write, FileShare.None)
            Dim copy As New PdfCopy(Doc, fs)
            ...
            Shell("cmd /C """ & My.Computer.FileSystem.SpecialDirectories.Desktop & "\EdoCtaDividendosATAF.pdf""", AppWinStyle.NormalNoFocus, False, 10000)
    ...
    Private Function generaQuery(Optional ByVal Anual As Boolean = False) As String
        ...
        sql &= " (SELECT NOMBRE_COMPAÑIA FROM CATALOGO_COMPAÑIAS WHERE COMPAÑIA = 2) AS ASOCIACION, B.BANCO" & vbCrLf
        ...
        sql &= " WHERE C.ASOCIACION=2" & vbCrLf
        If Anual Then
            sql &= " AND YEAR(C.FECHA_1) = " & Me.DateTimePicker1.Value.Year & vbCrLf
        Else
            sql &= " AND C.NUMERO_REPARTO=" & NumericUpDown11.Value.ToString() & vbCrLf
        End If
        If CheckBox1.Checked = True Then
            If RadioButton1.Checked = True Then
        ...
        If TextBox1.Text.Length > 0 Then
            ...
            sql &= " AND S.CODIGO_EMPLEADO = " & Val(TextBox1.Text) & vbCrLf
        End If
...
#End Region
    ...
    Private Sub bwCreatePDF_RunWorkerCompleted(ByVal sender As Object, ByVal e As System.ComponentModel.RunWorkerCompletedEventArgs) Handles bwCreatePDF.RunWorkerCompleted
    ...
    Private Sub Button13_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles Button13.Click
        ...
        Label2.Text = "Cancelando Proceso..."
    End Sub
End Class
