...
Public Class Activo_Fijo_Reports_es
    Dim i As Integer = 0
    ...
    Private Sub Activo_Fijo_Reports_es_Load(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles MyBase.Load
    ...
    Private Sub Activo_Fijo_Reports_es_Paint(ByVal sender As System.Object, ByVal e As System.Windows.Forms.PaintEventArgs) Handles MyBase.Paint
    ...
    Private Sub procesoTpoBien()
        ...
        If _tipos_bien.Length > 0 Then
            ...
            Dim _campo As String = IIf(Me.radAdquisicion.Checked, "B.FECHA_ADQUICICION", "B.FECHABAJA")
            Dim _condicion_baja As String = IIf(Me.radAdquisicion.Checked, " AND B.BAJA_TOTAL=" & IIf(Me.chkDeBaja.Checked, "1 ", "0 "), " AND B.BAJA_TOTAL=1 ")

            sql = "SELECT B.BIEN,B.DESCRIPCION_BIEN, (SELECT DESCRIPCION_CLASIFICACION FROM dbo.CONTABILIDAD_ACTIVO_FIJO_CLASIFICACION WHERE CLASIFICACION = B.CLASIFICACION) AS CLASIFICACION,(SELECT DESCRPCION FROM dbo.CONTABILIDAD_ACTIVO_FIJO_TIPO_BIEN WHERE TIPO_BIEN = B.TIPO_BIEN) As TIPO, (SELECT DESCRIPCION_UBICACION FROM dbo.CONTABILIDAD_ACTIVO_FIJO_UBICACION WHERE UBICACION = B.UBICACION) AS DESCRIPCION_UBICACION, B.CODIGO FROM dbo.CONTABILIDAD_ACTIVO_FIJO_BIEN B WHERE B.COMPAÑIA=" & Compañia & _depreciable & " AND CONVERT(date, " & _campo & ", 103) BETWEEN CONVERT(date, '" & Me.dtpDesde.Value.ToShortDateString() & "', 103) AND CONVERT(date, '" & Me.dtpHasta.Value.ToShortDateString() & "', 103) " & _condicion_baja & " AND B.TIPO_BIEN IN (" & _tipos_bien & ")"
    ...
    Private Sub procesoUbicaciones()
        ...
        If _codigos_ubicacion.Length > 0 Then
            ...
            If _tipos_bien.Length > 0 Then
                sql = "SELECT B.BIEN,B.DESCRIPCION_BIEN, (SELECT DESCRIPCION_CLASIFICACION FROM dbo.CONTABILIDAD_ACTIVO_FIJO_CLASIFICACION WHERE CLASIFICACION = B.CLASIFICACION) AS CLASIFICACION,(SELECT DESCRPCION FROM dbo.CONTABILIDAD_ACTIVO_FIJO_TIPO_BIEN WHERE TIPO_BIEN = B.TIPO_BIEN) As TIPO, (SELECT DESCRIPCION_UBICACION FROM dbo.CONTABILIDAD_ACTIVO_FIJO_UBICACION WHERE UBICACION = B.UBICACION) AS DESCRIPCION_UBICACION, B.CODIGO FROM dbo.CONTABILIDAD_ACTIVO_FIJO_BIEN B WHERE B.COMPAÑIA=" & Compañia & _depreciable & " AND CONVERT(date, " & _campo & ", 103) BETWEEN CONVERT(date, '" & Me.dtpDesde.Value.ToShortDateString() & "', 103) AND CONVERT(date, '" & Me.dtpHasta.Value.ToShortDateString() & "', 103) AND B.BAJA_TOTAL=" & IIf(Me.chkDeBaja.Checked, "1", "0") & _condicion_baja & " AND B.TIPO_BIEN IN (" & _tipos_bien & ") AND B.UBICACION IN (" & _codigos_ubicacion & ")"
            Else
                sql = "SELECT B.BIEN,B.DESCRIPCION_BIEN, (SELECT DESCRIPCION_CLASIFICACION FROM dbo.CONTABILIDAD_ACTIVO_FIJO_CLASIFICACION WHERE CLASIFICACION = B.CLASIFICACION) AS CLASIFICACION,(SELECT DESCRPCION FROM dbo.CONTABILIDAD_ACTIVO_FIJO_TIPO_BIEN WHERE TIPO_BIEN = B.TIPO_BIEN) As TIPO, (SELECT DESCRIPCION_UBICACION FROM dbo.CONTABILIDAD_ACTIVO_FIJO_UBICACION WHERE UBICACION = B.UBICACION) AS DESCRIPCION_UBICACION, B.CODIGO FROM dbo.CONTABILIDAD_ACTIVO_FIJO_BIEN B WHERE B.COMPAÑIA=" & Compañia & _depreciable & " AND CONVERT(date, " & _campo & ", 103) BETWEEN CONVERT(date, '" & Me.dtpDesde.Value.ToShortDateString() & "', 103) AND CONVERT(date, '" & Me.dtpHasta.Value.ToShortDateString() & "', 103) AND B.BAJA_TOTAL=" & IIf(Me.chkDeBaja.Checked, "1", "0") & _condicion_baja & " AND B.UBICACION IN (" & _codigos_ubicacion & ")"
            End If
    ...
    Private Sub btnPrintReport_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles btnPrintReport.Click
        If Me.radAdquisicion.Checked Then
            If dtpDesde.Value > dtpHasta.Value Then
                MessageBox.Show("Fechas Incorrectas...", "SISTEMA", MessageBoxButtons.OK, MessageBoxIcon.Warning)
                Return
            End If
            ...
            If _codigo_bienes.Length > 0 Then
                ...
                _verreport.ReportesActivoFijoAdquisicion(dtpDesde.Value.ToShortDateString(), dtpHasta.Value.ToShortDateString(), IIf(radFinanciero.Checked, "financiero", "fiscal"), IIf(Me.radCatAmbos.Checked, 3, IIf(Me.radDepreciable.Checked, 1, 0)), IIf(Me.chkDeBaja.Checked, 1, 0), _codigo_bienes, _codigos_ubicacion)
        ...
        If Me.radBaja.Checked Then
            If dtpDesde.Value > dtpDesde.Value Then
                MessageBox.Show("Fechas Incorrectas...", "SISTEMA", MessageBoxButtons.OK, MessageBoxIcon.Warning)
                Return
            End If
            Dim _codigo_bienes As String = String.Empty
            ...
            If _codigo_bienes.Length > 0 Then
                Me.btnPrintReport.Visible = False
                Dim _verreport As New frmReportes_Ver()
                _verreport.ReportesActivoFijoBajas(dtpDesde.Value.ToShortDateString(), dtpHasta.Value.ToShortDateString(), IIf(radFinanciero.Checked, "financiero", "fiscal"), IIf(Me.radCatAmbos.Checked, 3, IIf(Me.radDepreciable.Checked, 1, 0)), IIf(Me.chkDeBaja.Checked, 1, 0), _codigo_bienes, _codigos_ubicacion)
                _verreport.ShowDialog()
                Me.btnPrintReport.Visible = True
            Else
                MessageBox.Show("Seleccione los Bienes a incluir en el Reporte", "SISTEMA", MessageBoxButtons.OK, MessageBoxIcon.Exclamation)
    ...
    Private Sub radBaja_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles radBaja.Click
        Me.resetear()
    End Sub

    Private Sub radAdquisicion_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles radAdquisicion.Click
        Me.resetear()
    End Sub
