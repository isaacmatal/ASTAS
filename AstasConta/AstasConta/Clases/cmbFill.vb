...
Public Class cmbFill
    ...
    Public Function devuelveCadenaConexion()
        ...
        Return New SqlConnection(CnnStrBldr.ConnectionString)
    End Function
        ...
        Try
            ...
            For i As Integer = 0 To (Table.Rows.Count - 1)
                If Table.Rows(i).Item("TIPO_PROVEEDOR") = contribuyente Then
                    If flag = 1 Then
                        Return Table.Rows(i).Item("PROCENTAJE_RETENCION") / 100
                    ElseIf flag = 2 Then
                        Return Table.Rows(i).Item("PORCENTAJE_PERCEPCION") / 100
                    End If
                End If
            Next
            Return 0
    ...
    Public Sub ajustarGrid(ByVal numcolumns As Integer, ByVal dgv As DataGridView)
        ...
        For i As Integer = 1 To numcolumns
            dgv.Columns.Item(i - 1).AutoSizeMode = DataGridViewAutoSizeColumnMode.DisplayedCells
        Next
    ...
    Public Function GeneraCorrelativo(ByVal Compañia, ByVal TipoDocumento, ByVal Año, ByVal Mes) As Integer
        ...
        Try
            Conexion_.Open()
            sql = "Execute sp_CONTABILIDAD_CORRELATIVOS_DOCUMENTOS "
            sql &= Compañia
            sql &= ", '" & TipoDocumento & "'"
            sql &= ", " & Año
            sql &= ", " & Mes
    ...
    Public Sub anulacionPartida(ByVal Compañia As Integer, ByVal Transaccion As Integer, ByVal TransaccionAnulada As Integer, ByVal PartidaAnulada As Integer, ByVal FechaAnulacion As String, ByVal Motivo As String)
        ...
        Try
            ...
            sql &= ",@TRANSACCION_ANULA    = " & TransaccionAnulada
            sql &= ",@PARTIDA_ANULA        = " & PartidaAnulada
            sql &= ",@FECHA_CONTABLE_ANULA = '" & FechaAnulacion & "'"
            sql &= ",@USUARIO              = '" & Usuario & "'"
            sql &= ",@MOTIVO_ANULACION     = '" & Motivo & "'"
            Comando_ = New SqlCommand(sql, Conexion_)
